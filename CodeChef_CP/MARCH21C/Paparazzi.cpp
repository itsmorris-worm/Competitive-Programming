...
#define r0 return 0
#define r1 return 1
#define testcases int asdflkj; cin>>asdflkj;
#define endl "\n"
...
//input done
    ...
    for2(j,n)
    {
        jabtak(Zi>=2)
        {
            dbl m1 = ((dbl)h[Zi-1].Yi - (dbl)h[Zi-2].Yi)/((dbl)h[Zi-1].Xi - (dbl)h[Zi-2].Xi);
            dbl m2 = ((dbl)M[j].Yi - (dbl)h[Zi-1].Yi)/((dbl)M[j].Xi - (dbl)h[Zi-1].Xi);
            if(m1>m2)
                break;
            else
                {h.pob(); Zi--;}
        ...
        h.pb(M[j]);
        Zi++;
        res=max(res,h[Zi-1].Xi-h[Zi-2].Xi);
    }
    cout<<res<<endl;
    }
}
