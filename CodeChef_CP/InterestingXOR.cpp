...
long long int jadoo(long long int num) 
{ 
    // calculating left most set bits 
    long long left_most = log2(num); 
    //assign same to A
    long long int A=1;
  
    for (int i = 0; i < left_most; i++)  
       {
        long long int temp=(left_most-1-i);
        if((1<< temp)==(num & (1 << temp)))  
            A = (A<<1);
        else
            A = ((A<<1)|1);
       }
...
{ 
    long long int c,d;
    cin>>c;
    long long int A1=jadoo(c);
    long long int B=(A1^c);
    cout<<A1*B<<endl;
}
