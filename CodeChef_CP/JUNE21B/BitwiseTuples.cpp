...
//------------------------------------
ll int power(ll int x,ll int y,ll int MODI)
{
    ll int res=1;
    while(y)
    {
...
{
   ll int N;
   vector<ll int> X_co,Y_co;
   for(int i=0;i<N;i++)
   {
       ll int input1,input2;
       cin>>input1>>input2;
       X_co.push_back(input1);
       Y_co.push_back(input2);
   }
    if(N==1)
        cout<<0<<endl;
    else if(N%2==0)
    {
        bool flag=false;
        for(int i=0;i<X_co.size()-1;i+=2)
        {
            if((X_co[i]==X_co[i+1]) || (Y_co[i]==Y_co[i+1]))
                continue;
            else
                {
                    flag=true;
        ...
        if(!flag)
            ...
            {
            ll int length=*max_element(X_co.begin(),X_co.end())-1;
            ll int width=*max_element(Y_co.begin(),Y_co.end())-1;
